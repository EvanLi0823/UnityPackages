...
namespace RealYou.Utility.LinkList
{
    public class DelegateList<T>
    {
        ...
        public DelegateList(Func<Action<T>, LinkedListNode<Action<T>>> acquireFunc,
            Action<LinkedListNode<Action<T>>> releaseFunc)
        {
            if (acquireFunc == null)
        ...
        public static DelegateList<T> CreateWithGlobalCache()
        {
            ...
            return new DelegateList<T>(GlobalLinkedListNodeCache<Action<T>>.Acquire,
                GlobalLinkedListNodeCache<Action<T>>.Release);
        }
    }
}